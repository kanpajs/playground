(function() {
    ...
    run: function(template, data) {
      ...
      for (var i = 0; i < template.length; i++) {
        ...
        if (func.name === "#if" || func.name === "#elseif") {
          ...
          var res = TRANSFORM.fillout(data, "{{" + expression + "}}").data;
          if (res === "{{" + expression + "}}") {
      ...
      if (typeof template === "string") {
        ...
        if (Helper.is_template(template)) {
          ...
          if (include_string_re.test(template)) {
            ...
            let partial_template = TRANSFORM.templates[fun.expression];

            result = TRANSFORM.run(partial_template, data);
            ...
            // non-#include
            result = TRANSFORM.fillout(data, template).data;
          }
        } else {
          result = template;
        }
          ...
          if (Helper.is_template(key)) {
            ...
            if (fun) {
              if (fun.name === "#include") {
                ...
                if (
                  Helper.is_array(template[key]) &&
                  template[key].length == 2
                ) {
                ...
                for (let k in newData) {
                  var res = TRANSFORM.run(temp, { key: k, value: newData[k] });
                  result.push(res);
                }
                ...
                if (Helper.is_array(template[key])) {
                  ...
                  // clean up $in_dex from the result
                  // necessary because #merge merges multiple objects into one,
                  // and one of them may be 'this', in which case the $in_dex attribute
                  // will have snuck into the final result
                  if (typeof data === "object") {
                    delete result["$in_dex"];
                    ...
                    delete String.prototype.$in_dex;
                    delete Number.prototype.$in_dex;
                    delete Function.prototype.$in_dex;
                    delete Array.prototype.$in_dex;
                    delete Boolean.prototype.$in_dex;
                ...
                if (newData && Helper.is_array(newData)) {
                  ...
                  for (var index = 0; index < newData.length; index++) {
                    // temporarily set $in_dex
                    if (typeof newData[index] === "object") {
                      newData[index]["$in_dex"] = index;
                      newData[index]["$this"] = newData[index];
                      // #let handling

                      for (var declared_vars in TRANSFORM.memory) {
                        originals[declared_vars] =
                          newData[index][declared_vars]; // Modified by Jakub Mifek
                        newData[index][declared_vars] =
                          TRANSFORM.memory[declared_vars];
                      }
                    } else {
                      String.prototype.$in_dex = index;
                      Number.prototype.$in_dex = index;
                      Function.prototype.$in_dex = index;
                      Array.prototype.$in_dex = index;
                      Boolean.prototype.$in_dex = index;
                      // #let handling
                      for (var declared_vars in TRANSFORM.memory) {
                    ...
                    // run
                    var loop_item = TRANSFORM.run(
                      template[key],
                      newData[index]
                    );

                    // clean up $in_dex and $this
                    if (typeof newData[index] === "object") {
                      delete newData[index]["$in_dex"];
              ...
              var k = TRANSFORM.fillout(data, key).data;
              var v = TRANSFORM.fillout(data, template[key]).data;
            ...
            if (typeof template[key] === "string") {
              ...
              if (fun && fun.name === "#?") {
                ...
                if (
                  filled.data === "{{" + fun.expression + "}}" ||
                  (!filled.data && filled.data_type !== "boolean")
                ) {
                  // case 1.
    ...
    _fillout: function(options) {
      ...
      try {
        ...
        if (data) {
          ...
          if (
            ["number", "string", "array", "boolean", "function"].indexOf(
              data_type === -1
            )
          ) {
            data.$root = root;
          ...
          if (evaluated || evaluated == 0 || evaluated === false) {
            ...
            if (evaluated || evaluated == 0 || evaluated === false) {
              ...
              if (template) {
                ...
                if (full_re.test(template)) {
                  return {
                    data: evaluated,
                    data_type: typeof evaluated,
                  };
                } else {
                  return {
                    data: template.replace(variable, evaluated),
                    data_type: data_type,
                  };
                }
              } else {
              ...
              if (template) {
                ...
                if (full_re.test(template)) {
                  ...
                  return {
                    data: template.replace(variable, ""),
                    data_type: data_type,
                  };
                }
              } else {
      ...
      if (SELECT.$selected && SELECT.$selected.length > 0) {
        SELECT.$selected
          ...
          .forEach(function(selection) {
            ...
            SELECT.$selected_root = Helper.resolve(
              SELECT.$selected_root,
              selection.path,
              parsed_object
            );
      ...
      if (SELECT.$selected && SELECT.$selected.length > 0) {
        SELECT.$selected
          ...
          .forEach(function(selection) {
            ...
            // apply the result to root
            SELECT.$template_root = Helper.resolve(
              SELECT.$template_root,
              selection.path,
              parsed_object
            );
  ...
  var setTemplates = function(templates) {
    for (let template in templates)
      TRANSFORM.templates[template] = templates[template];
